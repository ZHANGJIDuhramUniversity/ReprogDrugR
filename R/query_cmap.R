...
query_cmap <- function(signature, db = "lincs", n_top = 100, tau = FALSE, workers = 1, add_annotations = TRUE) {
  ...
  .convert_genes <- function(genes) {
    ...
    genes <- unique(as.character(genes)[!is.na(as.character(genes)) & as.character(genes) != ""])
    if (length(genes) == 0) return(character(0))
  ...
  if (length(up) == 0) warning("No valid up-regulated genes after conversion.")
  if (length(down) == 0) warning("No valid down-regulated genes after conversion.")
  ...
  result <- tryCatch(
    ...
    error = function(e) {
      stop("CMap/LINCS query failed. Original error: ", e$message)
    }
  )
    ...
    result_df <- tryCatch(
      {
        ...
        lincs_pert_info <- as.data.frame(get("lincs_pert_info", envir = annot_env))
        ...
        overlapping_cols <- intersect(colnames(result_df), setdiff(colnames(annot_long), "join_key"))
        if (length(overlapping_cols) > 0) {
          annot_long <- annot_long[, !colnames(annot_long) %in% overlapping_cols, drop = FALSE]
        }

        merged_df <- merge(result_df, annot_long, by.x = "pert", by.y = "join_key",
      ...
      error = function(e) {
        warning("LINCS annotation failed. Returning unannotated results. Original error: ", e$message)
        result_df$.rank <- NULL
        result_df
      }
    )
